...
const string &Date::ToString() const
{
	...
	if(date && (str.empty() || epoch != epochInTranslating))
	{
		...
		const string &m = shortNameOfMonth[month];
...
string Date::LongString() const
{
	...
	const string &month = fullNameOfMonth[m];
	const size_t d = Day();
	const string &day = dayOfMonth[d];
	return Format::StringF({dateStringInConversationFormat, day, month});
}
