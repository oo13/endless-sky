...
namespace {
	...
	set<Person*> &GetListOfTranslation()
	{
		static set<Person*> *listOfTranslation(new set<Person*>);
		return *listOfTranslation;
	}
	
	// The Hook of translation.
	function<void()> updateCatalog([](){
		for(auto it : GetListOfTranslation())
...
{
	GetListOfTranslation().erase(this);
}
...
void Person::Load(const DataNode &node)
{
	...
	UpdateTranslation();
	if(IsTranslating())
		GetListOfTranslation().insert(this);
}
