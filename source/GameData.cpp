...
namespace {
	...
	bool IsSourceDirectory(const string &path)
	{
		return Files::Exists(path + "data") || Files::Exists(path + "fonts") || Files::Exists(path + "images")
			|| Files::Exists(path + "locales") || Files::Exists(path + "sounds");
	}
}
...
void GameData::LoadSources()
{
	...
	for(auto it = sources.begin() + 1; it != sources.end(); ++it)
	{
		...
		string aboutText = Files::Read(*it + "about.txt");
		// Remove all white spaces at the end of the file.
		while(isspace(static_cast<unsigned char>(aboutText.back())))
			aboutText.pop_back();
		plugins[name] = T_(std::move(aboutText));
