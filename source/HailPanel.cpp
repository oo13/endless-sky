	...
	: player(player), planet(object->GetPlanet()), sprite(object->GetSprite()), facing(object->Facing())
{
	...
	if(planet)
		// TRANSLATORS: %1%: government, %2%: planet's noun, %3%: planet name
...
bool HailPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command)
{
	...
	else if(key == 't' && hasLanguage && planet)
	{
		if(GameData::GetPolitics().HasDominated(planet))
		{
			...
			player.Conditions().erase("tribute: " + planet->Identifier());
	...
	else if((key == 'b' || key == 'o') && hasLanguage)
	{
		...
		else if(bribe)
		{
			if(ship)
			{
				...
				// TRANSLATORS: %1%: government, %2%: bribe
			...
			{
				planet->Bribe();
				// TRANSLATORS: %1%: planet, %2%: bribe
				Messages::Add(Format::StringF({T("You bribed the authorities on %1% %2% credits "
					"to permit you to land."), planet->Name(), Format::Number(bribe)}));
			}
