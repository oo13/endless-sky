	...
	for(const auto &ship : ships)
		if(ship->GetSystem() == system && !ship->IsDisabled() && !ship->IsParked())
		{
			...
			for(auto &bay : ship->Bays())
			{
				...
				if(bay.ship)
				{
					Files::LogError("Expected bay to be empty for " + ship->ModelName() + ": "
						+ FontUtilities::Unescape(ship->Name()));
...
void PlayerInfo::ValidateLoad()
{
	...
	for(auto &&ship : ships)
	{
		if(!ship->GetSystem() || !ship->GetSystem()->IsValid())
		{
			...
			Files::LogError("Warning: player ship \"" + FontUtilities::Unescape(ship->Name())
		...
		{
			ship->SetPlanet(planet);
			Files::LogError("Warning: in-system player ship \"" + FontUtilities::Unescape(ship->Name())
