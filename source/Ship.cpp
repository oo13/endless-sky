	...
	else if(requiredCrew && static_cast<int>(Random::Int(requiredCrew)) >= Crew())
	{
		...
		if(parent.lock() || !isYours)
			Messages::Add(Format::StringF(T("The %1% is moving erratically because "
				"there are not enough crew to pilot it."), name.Str()), false);
		else
			Messages::Add(T("Your ship is moving erratically because you do not have enough crew to pilot it."), false);
	}
	else
		pilotOkay = 30;
