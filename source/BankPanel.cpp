...
namespace {
	...
	const T_ HEADING[6] =
	{
		T_("Type", "BankPanel HEADING"),
		T_("Principal", "BankPanel HEADING"),
		T_("Interest", "BankPanel HEADING"),
		T_("Term", "BankPanel HEADING"),
		T_("Payment", "BankPanel HEADING"),
		T_("", "BankPanel HEADING")
	};
...
void BankPanel::Draw()
{
	...
	for(const auto &heading : HEADING)
		table.Draw(heading.Str());
	table.DrawGap(5);
	...
	if(maintenance || maintenanceDue)
	{
		...
		table.Draw(T("Maintenance", "BankPanel"));
		if(maintenanceDue)
		{
	...
	if(income[0] || income[1])
	{
		...
		static const T_ LABEL[] = {T_("", "BankPanel LABEL"), T_("Your Salary Income"), T_("Your Tribute Income"),
			T_("Your Salary and Tribute Income")};
		const auto incomeLayout = Layout(310, Truncate::BACK);
		table.DrawCustom({LABEL[(income[0] != 0) + 2 * (income[1] != 0)].Str(), incomeLayout});
	...
	table.Advance(3);
	table.Draw(T("total:"), selected);
	table.Draw(totalPayment, unselected);
	table.Advance();
	
	// Draw the credit score.
	table.DrawAt(Point(0., FIRST_Y + 210.));
	const string credit = Format::StringF(T("Your credit score is %1%."), to_string(player.Accounts().CreditScore()));
	...
	string amount;
	if(!qualify)
		amount = T("You do not qualify for further loans at this time.");
	else
		amount = Format::StringF(T("You qualify for a new loan of up to %1% credits."), Format::Credits(qualify));
...
bool BankPanel::KeyDown(SDL_Keycode key, Uint16 mod, const Command &command, bool isNewPress)
{
	...
	else if(key == SDLK_RETURN && selectedRow < mortgageRows)
	{
		GetUI()->Push(new Dialog(this, &BankPanel::PayExtra,
			T("Paying off part of this debt will reduce your daily payments and the "
