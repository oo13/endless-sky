...
namespace {
	...
	// Column headings.
	const T_ HEADING[6] = {T_("Type"), T_("Principal"), T_("Interest"), T_("Term"), T_("Payment"), T_("", "BankPanel HEADING")};
...
void BankPanel::Draw()
{
	...
	string credit = Format::StringF({T("Your credit score is %1%."), to_string(player.Accounts().CreditScore())});
	table.Draw(credit);
	table.Advance(5);
