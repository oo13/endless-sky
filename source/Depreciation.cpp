	...
	out.BeginChild();
	{
		...
		WriteSorted(ships,
			[](const ShipElement *lhs, const ShipElement *rhs)
				{ return lhs->first->ModelTrueName() < rhs->first->ModelTrueName(); },
			[=, &out](const ShipElement &sit)
			{
				out.Write("ship", sit.first->ModelTrueName());
				out.BeginChild();
				{
...
void Depreciation::Init(const vector<shared_ptr<Ship>> &fleet, int day)
{
	...
	{
		const Ship *base = GameData::Ships().Get(ship->ModelTrueName());
		++ships[base][day];
...
int64_t Depreciation::Value(const Ship *ship, int day, int count) const
{
	...
	ship = GameData::Ships().Get(ship->ModelTrueName());
