...
void Depreciation::Save(DataWriter &out, int day) const
{
	...
	out.BeginChild();
	{
		...
		{
			out.Write("ship", sit.first->ModelIdentifier());
			out.BeginChild();
			{
		...
		{
			out.Write("outfit", oit.first->Identifier());
			out.BeginChild();
			{
...
void Depreciation::Init(const vector<shared_ptr<Ship>> &fleet, int day)
{
	...
	{
		const Ship *base = GameData::Ships().Get(ship->ModelIdentifier());
		++ships[base][day];
...
int64_t Depreciation::Value(const Ship *ship, int day, int count) const
{
	...
	ship = GameData::Ships().Get(ship->ModelIdentifier());
